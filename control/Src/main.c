...
int main(void)
{
  ...
  /**Initialize the timer to flash the tail light**/
  TIMER_CONTROL_SETTING();
  /**Register the tail light device**/
  LIGHT_CONTROL_SETTING();
  /**System is ready, then send boot message later**/
  MESSAGE_CONTROL_SETTING();
  /**Open timer to monitor the UART Protocol's stability between the controller and dashboard!**/
  TX_TIMER_CONTROL_SETTING();
  /**Start monitoring the UART connection's status!**/
  msg_retransmission_control();
  /*The Tail Light is ON in Default*/
  Tail_Light_Control(0x01);
  /**The system is OK. Send the message to notify the dash board!**/
  //send_alert_message();
  while (1)
  {
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : SAFETY_LIGHT_Pin HEARTBEAT_Pin POWER_INDICATOR_Pin */
